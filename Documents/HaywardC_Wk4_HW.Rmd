...
library("opusminer")
apriori(bookbaskets)
itemLabels(bookbaskets)

#bookbaskets_noSeries <- apriori(bookbaskets, parameter = list(support = 0.0005, 
                                                            #confidence = 0.9, minlen = 2, #maxlen = 10), appearance = list(none = c("token=!yo!", default="both")))
#opus(bookbaskets, k = 100, format = "data.frame", sep = " ", print_closures = FALSE, #filter_itemsets = TRUE, search_by_lift = FALSE, correct_for_mult_compare = TRUE, #redundancy_tests = TRUE)
...
```{r}
inspect(bookbaskets_noAnimus[1:10]) 
```
...
```{r}
bookbaskets_sorted <- sort(bookbaskets_noAnimus, by = "lift")
...
```{r}
bookbaskets_sorted <-sort(bookbaskets_noAnimus, by = c("lift", "confidence"))
...
```{r}
write(bookbasketsrules_sorted, file = "C:/Users/cindyHayward/Desktop/ADM/Week_Four/Week4_GIT/Documents/bookbasketsrules.csv",
